...
export function initIRRS() {
    ...
    if (missing.length) {
      showFatalError(
        `Page section not found in HTML: ${missing.join(", ")}\n` +
          `Check App.tsx has <section id="..."> with those IDs.`
      );
      return false;
    }
    return true;
  ...
  function canAccessPage(pageName) {
    ...
    if (pageName === "parentForm" || pageName === "parentTrack")
      return session.role === "parent";
    if (["staffDashboard", "staffView", "staffEdit", "approved"].includes(pageName))
  ...
  function showPage(name) {
    ...
    if (!pages[name]) {
      ...
      name = fallback;
      if (!pages[name]) {
        showFatalError(`Cannot render page "${name}" because section is missing in App.tsx.`);
        return;
      }
    }
  ...
  function findActiveRequestBySerial(ownerKey, serialRaw) {
    ...
    return (
      list.find((r) => normSerial(r.serial) === serial && activeStatuses.has(r.status)) || null
    );
  }
  ...
  function applyRoleAccess(role) {
    document
      .querySelectorAll('[data-role="it"]')
      .forEach((el) => el.classList.toggle("hidden", role !== "it"));
    document
      .querySelectorAll('[data-role="registrar"]')
  ...
  function computeKpi(list) {
    ...
    list.forEach((r) => {
      if (k[r.status] !== undefined) k[r.status] += 1;
    });
    return k;
  }
  ...
  function fillParentTrackDetails(r) {
    ...
    setText(
      "parentTrackStudent",
      r.studentName ? `${r.studentName} (${r.classRoom || "—"})` : "—"
    );
    ...
    tbody.innerHTML = list
      .map((r) => {
        ...
        <tr class="${rowClass}">
          ...
          <td class="px-5 py-4">${escapeHtml(serialShort)}${
          (r.serial || "").length > 12 ? "…" : ""
        }</td>
  ...
  function resetParentForm() {
    ["pParentName", "pPhone", "pStudentName", "pClassRoom", "pSerial", "pReason"].forEach(
      (id) => {
        const el = $(id);
        if (el) el.value = "";
      }
  ...
  function getNextActionFor(role, status) {
    ...
    if (role === "registrar") {
      if (status === "submitted") {
        return {
          label: "Approve & Forward to IT",
          toStatus: "pending_it",
          tip: "Registrar can approve and forward this request to IT (one click)."
        };
      }
      if (status === "pending_it")
        return { label: null, toStatus: null, tip: "Already forwarded to IT. Waiting for IT final approval." };
      if (status === "it_approved")
        return { label: null, toStatus: null, tip: "Completed. Print/PDF is available." };
    }
    ...
    if (role === "it") {
      if (status === "pending_it") {
        return {
          label: "Final Approve (IT)",
          toStatus: "it_approved",
          tip: "IT can final approve requests that are waiting for IT."
        };
      }
      if (status === "submitted")
        return { label: null, toStatus: null, tip: "Waiting for Registrar approval and forward first." };
      if (status === "it_approved")
        return { label: null, toStatus: null, tip: "Completed. Print/PDF is available." };
    }
  ...
  onClick("btnLogin", () => {
    ...
    if (!u || u.role !== role || u.password !== pass) {
      err?.classList.remove("hidden");
      if (err) err.textContent = "Invalid login credentials.";
      return;
    }
